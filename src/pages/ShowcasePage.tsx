          ...
          {showcaseItems.map((project, index) => {
            ...
            return (
              <motion.article
                ...
                viewport={{ once: true }}
                className={`group relative overflow-hidden rounded-[1.6rem] bg-gradient-to-br ${project.theme} p-4 text-white shadow-[0_26px_70px_-40px_rgba(15,23,42,0.9)] sm:rounded-[2rem] sm:p-6`}
              >
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(255,255,255,0.16),transparent_28%)] opacity-90" />

                <div className="relative z-10 flex h-full min-h-[460px] flex-col sm:min-h-[520px]">
                  <div className="flex items-start justify-between gap-4">
                    <div className="inline-flex h-12 w-12 items-center justify-center rounded-2xl border border-white/15 bg-white/10 backdrop-blur-sm sm:h-14 sm:w-14">
                  ...
                  <div className="mt-6 max-w-md transition duration-300 xl:mt-8 xl:group-hover:pointer-events-none xl:group-hover:translate-y-3 xl:group-hover:opacity-0">
                    <p className="text-xs font-semibold uppercase tracking-[0.18em] text-white/55">{t.hint}</p>
                    <h2 className="mt-3 text-[1.9rem] font-display font-extrabold tracking-tight text-white sm:text-4xl">
                      {project.title}
                    </h2>
                    ...
                    <div className="mt-6 inline-flex items-center text-sm font-semibold text-white sm:mt-8">
                      {t.view}
                      <ArrowUpRight className="ml-2 h-4 w-4" />
                    </div>
                  </div>

                  <div className="mt-auto flex flex-col gap-3 pt-5 sm:flex-row sm:flex-wrap sm:items-center sm:pt-6">
                    {project.demoExternal ? (
                      <a
                        href={project.demoPath}
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex w-full items-center justify-center rounded-full bg-white px-5 py-3 text-sm font-semibold text-slate-900 transition hover:-translate-y-0.5 sm:w-auto sm:py-2.5"
                      >
                        {t.openDemo}
                        <ArrowUpRight className="ml-2 h-4 w-4" />
                      </a>
                    ...
                    <span className="hidden text-xs font-medium uppercase tracking-[0.18em] text-white/50 sm:inline-flex">
                      {t.hoverNote}
                    </span>
                  </div>

                  <div className="mt-5 xl:absolute xl:inset-x-6 xl:bottom-20">
                    <div className="h-[220px] rounded-[1.3rem] border border-white/10 bg-[#0b1220]/45 p-2.5 backdrop-blur-sm transition duration-300 sm:h-[280px] sm:rounded-[1.6rem] sm:p-3 xl:opacity-0 xl:translate-y-4 xl:group-hover:opacity-100 xl:group-hover:translate-y-0">
                      {project.preview}
                    </div>
                  </div>
                </div>
